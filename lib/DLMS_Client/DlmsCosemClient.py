...
class DlmsCosemClient:
	...
	def __init__(self, port, baudrate, parity, bytesize, stopbits, timeout, inactivity_timeout, login_retry, meter_addr, client_nb):
		import sys
	...
	def start_inactivity(self):
		self.is_inactive = False
		self.timer_count = 0
		Thread(target = self.count_inactivity, args = (), daemon=True).start()
	...
	def aarq_msg(self, password, sys_T = None, GUEK = None, GAK = None, start_IC = None):
		auth_mech = mechanism.LOWEST_LEVEL
		if len(password) > 0:
		...
		if password != "":
			...
			password = []
		if sys_T != None and GUEK != None:
			auth_mech = mechanism.HIGH_LEVEL_GMAC
		aarq_frame = self.dlms.get_AARQ_frame(auth_mech, password, sys_T, GUEK, GAK, start_IC)
	...
	def send_HLS_Pass3(self, auth_mech):
		pass3_frame = []
		if auth_mech == mechanism.HIGH_LEVEL_GMAC:
			pass3_frame = self.dlms.get_Reply_to_HLS_frame()
	...
	def read_msg(self, is_optic = False):
		...
		while self.retry_counter < self.num_of_retry:
			...
			while not self.is_inactive:
				...
				if (len(check_serial) != 0) and not(receive_in_progress):
	...
	def client_login_optic(self, password = ""):
		self.last_send_msg = []
		login_success = False
		...
		if self.read_msg(True) == True:
			...
			if self.read_msg(True) == True:
				...
				if password == "":
					login_success = self.client_login()
				else:
					login_success = self.client_login(password)
		return login_success

	def client_login_HLS(self, auth_mech, sys_T, GUEK, GAK, start_IC):
		self.last_send_msg = []
		login_success = False
		try:
			if len(sys_T) != 16 or len(GUEK) != 32 or len(GAK) != 32:
				raise ValueError # Bad length parameter
			self.snrm_msg()
			check_resp = self.read_msg()
			if check_resp == frameError.NO_ERROR:
				self.retry_counter = 0
				self.aarq_msg([], sys_T, GUEK, GAK, start_IC)
				check_resp = self.read_msg()
				if check_resp == frameError.NO_ERROR:
					is_next = self.aare_check(self.hdlc.rx_frameInfo)
					if is_next:
						self.send_HLS_Pass3(auth_mech)
						check_resp = self.read_msg()
						if check_resp == frameError.NO_ERROR:
			...
			print("Bad Length Parameter")
			login_success = False
		return login_success

	def client_login(self, password = ""):
		self.last_send_msg = []
		login_success = False
		self.snrm_msg()
		check_resp = self.read_msg()
		...
		# else:
		# 	self.retry_counter += 1
		# 	if self.retry_counter <= self.num_of_retry:
		# 		login_success = self.client_login(password)
		# 	else:
		# 		login_success = False
		return login_success
	...
	def send_get_req_buffer(self, type, class_id, obis_code, att_id, sel_access_param):
		req_buffer = self.dlms.get_GET_REQ_frame(type, class_id, obis_code, att_id, sel_access_param)
	...
	def get_cosem_data(self, class_id, obis_code, att_id, sel_access_param = None, is_note = False):
		...
		if self.is_connected:
			self.send_get_req_buffer(ServiceTag.GET_REQUEST_NORMAL, class_id, obis_code, att_id, sel_access_param)
			...
			if len(frame_to_process) > 0:
				...
				while not is_finish:
					self.send_get_req_buffer(ServiceTag.GET_REQUEST_NEXT, class_id, obis_code, att_id, sel_access_param)
					get_resp_process = True
					frame_to_process = []
					while get_resp_process:
						get_resp_process, frame_to_process = self.read_get_resp_buffer()
	...
	def send_set_req_buffer(self, req_buffer):
		req_buffer = self.hdlc.getTxInformationField(req_buffer)
		seg_bit = segmentBit.SB_OFF
		if len(self.hdlc.tx_segmented_info) > 0:
			seg_bit = segmentBit.SB_ON
	...
	def read_set_resp_buffer(self):
		...
		if check == None:
			is_answered = False
			set_resp_progress = False
		return is_answered, set_resp_progress
	...
	def set_cosem_data(self, class_id, obis_code, att_id, dtype, value, is_note = False):
		...
		if self.is_connected:
			...
			while set_resp_progress:
				is_answered, set_resp_progress = self.read_set_resp_buffer()
			if is_answered:
				is_finish, result = self.dlms.check_SET_RESP_frame(self.hdlc.rx_frameInfo)
				if not is_finish:
	...
	def send_act_req_buffer(self, req_buffer):
		req_buffer = self.hdlc.getTxInformationField(req_buffer)
		response_buff = self.hdlc.getTxHdlcFrame(req_buffer, segmentBit.SB_OFF, self.dest_addr, self.src_addr, controlType.INFO, controlType.INFO, self.counter_R, self.counter_S, pollFinalBit.PF_ON)
		self.increment_S()
		self.send_msg(response_buff)
