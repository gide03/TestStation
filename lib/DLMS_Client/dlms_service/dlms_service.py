...
class DlmsService:
	...
	def set_security_param(self, sys_T, GUEK, GAK, invoc_count):
	...
	def get_AARQ_frame(self, auth_mech, auth_val, sys_T = None, GUEK = None, GAK = None, IC = None):
		buff = [AARQTag.AARQ]
		...
		if auth_mech != mechanism.LOWEST_LEVEL:
			if sys_T != None:				
				ap_title_field = [AARQTag.SysTitle, 0x0A, 0x04, 0x08] + self.conv_hexstring_to_list(sys_T)
			...
			if auth_mech == mechanism.LOW_LEVEL:
				...
				acse_req_buff.append(0x05)
				self.set_security_param(sys_T, GUEK, GAK, IC)
				auth_val = self.security.generate_challenge(32) # auth val in HLS is CtoS
	...
	def get_Reply_to_HLS_frame(self):
		response = None
		sec_param = None
		if self.auth_mech == mechanism.HIGH_LEVEL_GMAC:
			sec_param = [sc_byte.AUTH_ONLY, self.GUEK, self.GAK]
		buff = self.security.construct_HLS_Pass3(self.auth_mech, self.sys_TC, self.sys_TS, sec_param)
		if len(buff) > 0:
			act_buff = self.get_ACT_REQ_frame(15, "0;0;40;0;0;255", 1, CosemDataType.e_OCTET_STRING, buff)
			chipper_act_buff = self.security.encrypt_apdu(sc_byte.AUTH_ENCRYPT, self.GUEK, self.GAK, self.sys_TC, act_buff)
	...
	def check_Reply_to_HLS_response(self, buffer):
		...
		if self.auth_mech == mechanism.HIGH_LEVEL_GMAC:
			...
			plain_buff = plain_buff[8:]
			is_ok = self.security.verify_HLS_Pass4(self.auth_mech, self.sys_TC, self.sys_TS, sec_param, plain_buff)
		return is_ok
	...
	def check_GET_RESP_frame(self, buff):
		is_ok = True
		is_finish = True
		data_type = None
		data_result = None
	...
	def check_SET_RESP_frame(self, buff):
		is_finish = True
